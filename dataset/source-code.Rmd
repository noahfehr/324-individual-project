...
wordcloud(words=renewablebycountry$Country, freq = renewablebycountry$total, colors=brewer.pal(8, "Dark2"), scale=c(10,0.75))
```
```{r}
library(tidyverse)
library(ggplot2)
...
ui <- fluidPage(
  ...
  tabPanel("Overview", 
      ...
      fluidRow(
        column(width=2),
        column(plotOutput("cloud"), width = 8),
        column(width=2)
        ),
      fluidRow(
         column(width=2),
  ...
  tabPanel("Methods and Sources",
      ...
      fluidRow(
        column(width=2),
        column(
          p("(2) There is a loose positive relationship between total energy consumption and renewable energy production. Obvious outliers to this rule include Brazil and Canada, who produce high amounts of rewewable energy relative to their total energy consumption, and Iran, who produces less than expected."),
          width=8
          ),
        column(width=2)
      ),
