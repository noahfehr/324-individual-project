...
ui <- fluidPage(
  ...
  plotOutput("pie"),
  plotOutput("scatter", hover = hoverOpts("hover_spot")),
...
server <- function(input, output) {
  ...
  output$scatter <- renderPlot({
    ...
    geom_point() +
    geom_text(label=nooutliers$Country, nudge_x = 50)
    })
  
  ##Playing around with hovering
  output$info <- renderText({
    hover <- input[["hover_spot"]]
    point <- nearPoints(nooutliers, hover, threshold = 5, maxpoints = 1)
